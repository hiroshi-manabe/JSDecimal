...
Decimal.countValidNum = function(sig) {
	...
	for (var j = Decimal.decimal_digits_per_word - 1; j >= 0; --j) {
	    n /= 10;
	    if (sig[i] >= n) {
		return i * Decimal_digits_per_word + j + 1;
	    }
	}
    }
...
Decimal.prototype.addSub = function(another) {
    ...
    ret = abs_larger.absAddSub(abs_smaller, is_abs_sub);
...
Decimal.prototype.add = function(another) {
    return this.add_sub(another, False);
};

Decimal.prototype.sub = function(another) {
    return this.add_sub(another, True);
};
...
Decimal.prototype.mul = function() {
    var another_decimal = (another instanceof Decimal ? another :
			   new Decimal(another));
...
Decimal.prototype.div = function() {
	...
	if (tmp2 < Decimal.two_words) {
	    resutl -= Math.floor((Decimal.two_words - 1 - tmp2) /
    ...
    for (var i = 0; i < self_sig.length; ++i) {
	if (self_sig[i]) {
	    result_sig[0] += 1;
	    break;
	}
    ...
    var result_exp = (this.exp - another_decimal.exp +
		      (another_v - (self_v - added) + Decimal.n + 1) *
		      Decimal.decimal_digits_per_word);

    return Decimal.from_data(result_sig, result_exp,
			     this.is_minus != another.is_minus);
};
window.alert(1);
